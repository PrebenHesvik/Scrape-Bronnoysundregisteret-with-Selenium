...
class BrregScraper(Scraper):
    ...
    def searchable_xpaths(self):
        ...
        self.xpaths_to_search = {
            k: v for k, v in self.xpaths.items() if k not in do_not_search
        }
    ...
    def scrape_data(self, supplier_id, vat_num):
        ...
        self.driver.find_element(
            By.XPATH, self.xpaths.get("front-page-search-box")
        ).send_keys(vat_str)
        self.driver.find_element(
            By.XPATH, self.xpaths.get("front-page-search-box")
        ...
        if self.check_if_xpath_exists(self.xpaths.get("name")) == "":
            ...
            msg = self.check_if_xpath_exists(
                element, return_str="Could not find error message"
            )
            # msg = self.driver.find_element(By.XPATH, element).text
            console.print(
                f"[bold red]Warning:[/] [bold cyan]{supplier_id} {vat_num}[/]: {msg}"
            )
            logging.info(f"\n \t{supplier_id}\n \t{msg}")
            ...
            brreg_data = {
                k: self.check_if_xpath_exists(v, return_str=np.NaN)
                for k, v in self.xpaths_to_search.items()
            }
    ...
    def clean_address(df):
        ...
        zipped = zip(["street", "postal_code", "city"], [road, postal_code, city])
    ...
    def clean_postal_address(df):
        ...
        att = np.where(ser.str.len() == 3, ser.str[0], np.NaN)
